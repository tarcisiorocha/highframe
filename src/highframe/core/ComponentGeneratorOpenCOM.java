...
public class ComponentGeneratorOpenCOM implements IComponentGenerator {
	
	@Override
	public void generateComponent(Class<?> classe) {
		try {			
			//Classe criada para processar anotações e classe .java
			ClassReaderOpenCOM pc = new ClassReaderOpenCOM(classe);
			...
			Template t = ve.getTemplate("templates/templateOpenCOM.vm");
			
			//Método que retorna a assinatura a ser utilizada no component
			//LEMBRAR DE TIRAR O SYSTEM.CURRENTTIMEMILLIS()!!!!!!!!!!!!!!!!!!!!!!
			String nomeClasse = pc.getClassName();//+System.currentTimeMillis();
